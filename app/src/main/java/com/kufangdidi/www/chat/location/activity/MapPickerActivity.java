package com.kufangdidi.www.chat.location.activity;
...
import com.kufangdidi.www.chat.location.adapter.MapPickerAdapter;
...
public class MapPickerActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String targetId = getIntent().getStringExtra(BaseApplication.TARGET_ID);
        String targetAppKey = getIntent().getStringExtra(BaseApplication.TARGET_APP_KEY);
        long groupId = getIntent().getLongExtra(BaseApplication.GROUP_ID, 0);
                ...
                mBaiduMap.snapshotScope(rect, new BaiduMap.SnapshotReadyCallback() {
                    ...
                    public void onSnapshotReady(Bitmap bitmap) {
                        if (null != bitmap && null != conv) {
                            ...
                            setResult(BaseApplication.RESULT_CODE_SEND_LOCATION, intent);
                            finish();
                        } else {
                            Toast.makeText(MapPickerActivity.this, getApplicationContext().getString(R.string.send_location_error),
